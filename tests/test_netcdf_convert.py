...
def test_spl3smp_e_nested_variable_subsetted_annotated(
    temp_dir, logger, spl3smp_e_all_variables_subsetted_annotated_file
):
    """Ensure that tb_time_seconds in SPL3SMP_E granules
    is correctly transformed from datetime64[ns] to dtype float64,
    enabling GeoTIFF compatibility

    """
    test_file = pathlib.Path(
        temp_dir,
        spl3smp_e_all_variables_subsetted_annotated_file
    )
    ...
    assert pathlib.Path(results[0]).is_file(), "No file created."
    assert (
        basename(results[0]) == "Soil_Moisture_Retrieval_Data_AM_tb_time_seconds.tif"
    ), "Incorrect output file name"
    assert cog_validate(pathlib.Path(results[0]))[0]


SMAP_BANDS = [
    ['/Soil_Moisture_Retrieval_Data_AM/longitude', "EPSG:6933"],
    ['/Soil_Moisture_Retrieval_Data_AM/latitude', "EPSG:6933"],
    ['/Soil_Moisture_Retrieval_Data_AM/tb_time_seconds', "EPSG:6933"],
    ['/Soil_Moisture_Retrieval_Data_AM/surface_flag', "EPSG:6933"],
    ['/Soil_Moisture_Retrieval_Data_Polar_PM/surface_flag_pm', "EPSG:6931"],
    ['/Soil_Moisture_Retrieval_Data_Polar_PM/longitude_pm', "EPSG:6931"],
    ['/Soil_Moisture_Retrieval_Data_Polar_PM/latitude_pm', "EPSG:6931"],
]

# 2. Fix the decorator syntax: Use a string for the name and wrap the list in a list
@pytest.mark.parametrize("band_mapping", [SMAP_BANDS])
def test_spl3smp_e_all_variable_subsetted_annotated(
    band_mapping, temp_dir, logger, spl3smp_e_all_variables_subsetted_annotated_file
):
    # Sort the mapping so paths and CRS stay aligned
    band_mapping = sorted(band_mapping, key=lambda x: x[0])

    # Extract only the paths (strings) to pass to the converter
    in_paths = [item[0] for item in band_mapping]

    test_file = pathlib.Path(temp_dir, spl3smp_e_all_variables_subsetted_annotated_file)

    # Run converter (it usually returns a list of output file paths)
    results = netcdf_converter(
        test_file,
        pathlib.Path(temp_dir),
        in_paths,
    ...
    assert len(results) == 7, 'Incorrect number of output file names.'
    results.sort()

    # 3. Create expected filenames for comparison
    expected_filenames = [p.lstrip('/').replace('/', '_') for p in in_paths]

    out_bands = []
    # Zip results with the mapping to compare each file to its specific CRS
    for entry, (original_path, expected_crs) in zip(results, band_mapping):
        if pathlib.Path(entry).is_file():
            band_name = splitext(basename(entry))[0]
            out_bands.append(band_name)

            # Verify CRS matches the specific band
            actual_crs = cog_info(pathlib.Path(entry)).GEO.CRS
            assert actual_crs == expected_crs
            print(f'Variable: {original_path}')
            print(f'    Expected CRS: {expected_crs} | Actual: {actual_crs}')

    out_bands.sort()
    expected_filenames.sort()
    assert out_bands == expected_filenames, 'Incorrect output file names.'
